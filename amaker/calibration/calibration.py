...
class CameraCalibration:
    ...
    def  __init__(self, chessboard_images_path: str = './*.jpg', chessboard_size=(10, 7), destination_file="camera_calibration.npz", wait_time_ms=250):
    ...
    def calibrate_camera(self, square_size_mm: float = 30.0):
        ...
        try:
            ...
            for fname in images:
                ...
                ret, corners = cv.findChessboardCorners(gray, (self.chessboard_size[0]-1,self.chessboard_size[1]-1), None)
                ...
                if ret == True:
                    ...
                    corners2 = cv.cornerSubPix(gray,corners, (self.chessboard_size[0],self.chessboard_size[1]), (-1,-1), criteria)
                    image2D_points.append(corners2)
                    # Draw and display the corners
                    cv.imshow('Scanning for chessboard', img)
                    cv.waitKey(self.wait_time_ms)
                    img_corners = img.copy()
                    cv.drawChessboardCorners(img_corners, (self.chessboard_size[0]-1,self.chessboard_size[1]-1), corners2, ret)
