...
UI_VIDEO_TITLE = "Mission Unleash The Bricks - simulation"
...
class BotInfo():
    ...
    def __init__(self, name:str="microbot", id:int=None,
                  color_a=DEFAULT_BOT_COLOR_A,
                  color_b=DEFAULT_BOT_COLOR_B,
...
class VideoTracker():
    def __init__(self, camera_index:int=0,
                 bot_trackers:List[BotInfo]=None, 
                 serial_manager=None):


        
        # Handle serial communication through SerialManager
    ...
    def button_safety(self):
        ...
        if self.serial_manager:
            self.serial_manager.send_command("SAFETY\n")
    ...
    def _add_button(self, button, frame):
        ...
        cv2.putText(frame,
                    button['text'],
                    (button['x'] + BUTTON_PADDING, button['y'] + 37),
    ...
    def build_display_frame(self, input_frame, is_fullscreen, screen_width, screen_height, window_width, window_height, buttons=None):
        ...
        cv2.putText(display_frame, UI_VIDEO_TITLE, (20, 60),
                   cv2.FONT_HERSHEY_SIMPLEX, 2, TEXT_COLOR_WHITE, 5, cv2.LINE_AA)
    ...
    def start_tracking(self, recording:bool=False, window_name=WINDOW_TITLE):
        ...
        # Define buttons
        button_start = {'x': BUTTON_X_POS+BUTTON_MARGIN_X,
                       'y': BUTTON_MARGIN_Y, 
                       'w': BUTTON_WIDTH, 
                       'h': BUTTON_HEIGHT, 
                       'text': 'Start',
                       'clicked': False}
        button_stop = {'x': BUTTON_X_POS+BUTTON_MARGIN_X + BUTTON_WIDTH + BUTTON_SPACING,
                      'y': BUTTON_MARGIN_Y,
                      'w': BUTTON_WIDTH, 
                      'h': BUTTON_HEIGHT, 
                      'text': 'Stop',
        ...
        while True:
            ...
            for bot_tracker in self.bot_trackers:
                ...
                if bot_contours:
                    ...
                    if len(bot_tracker.get_trail()) > 1:
                        cv2.polylines(input_frame, [np.array(bot_tracker.get_trail())], False, 
            ...
            display_frame = self.build_display_frame(
                ...
                window_width,
                window_height,
                [button_start, button_stop, button_safety]
            )
...
if __name__ == "__main__":
    ...
    bot1 = BotInfo(name="bot1", 
                   color_a=(106, 40, 30), 
                   color_b=(150, 70, 60)
                   )
    
    # Create serial manager
    serial_manager = SerialManager(serial_port="/dev/ttyACM0", baud_rate=57600)
    
    # Create video tracker with serial manager
    vt = VideoTracker(camera_index=4, bot_trackers=[bot1], serial_manager=serial_manager)
